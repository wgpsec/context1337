...
//
// Returns space-joined token string for FTS5 indexing.
...
// TokenizeToString returns tokenized text as a space-joined string,
// suitable for direct insertion into FTS5 content fields.
func TokenizeToString(text string) string {
	tokens := Tokenize(text)
	if len(tokens) == 0 {
		return text // fallback to raw text if no tokens extracted
	}
